...
- `src/data/seed/route-segments.ts`: 3·4·7·11·12일 `trailViaIds`(주요 고개 1곳)
- `MapLinkCard`: Google 걷기 링크(FR-003) + 산길 경로 링크(`data-testid="trail-link"`) + 우회 경고 문구

## 8. Day별 GPX 다운로드 (2026-09-18 추가)
- `scripts/build-gpx.mjs` → `public/gpx/tmb2027-day-NN.gpx`(12) + `src/data/gpx-manifest.json`(dayId, file, lengthKm, ascendM, trackPoints, viaNodeIds, generatedAt, source)
